...
class OrderFilterSideSheet extends StatelessWidget {
  final OrderPortalType type;

  const OrderFilterSideSheet({super.key, required this.type});
  ...
  Widget build(BuildContext context) {
    return Consumer<OrderViewModel>(
      builder: (context, vm, child) {
        ...
        final options = vm.sideOptions;
        final selectedIndex = stateManager.selectedSide == null
            ? 0
            : options.indexOf(stateManager.selectedSide!);
        return Padding(
              ...
              Column(
                children: List.generate(options.length, (index) {
                  ...
                  return ListTile(
                    onTap: () {
                      vm.filter(
                        OrderFilterType.Side,
                        options[index] == AppStorageKey.all
                            ? null
                            : options[index],
                        type,
                      );
